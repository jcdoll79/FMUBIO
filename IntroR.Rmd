...
-   Objects
-   Data Types
-   Creating New Variables and Variable assignment
-   Functions
-   Packages
-   Coding Tips
-   Getting Help
...
```{r data_types}
#numeric, these data types can take on any number with decimals. 
#You will often use these with temperature, catch rate, weight, etc.
...
#integer, these data types can take on any whole without decimals. 
#You will often use these with counts of individuals.
...
#character, these data types represent string values. 
#You will often use these for captions and column headers.
...
A function include the function name followed by parentheses. The parentheses contains arguments that the function needs. For example, the mean() function requires a vector within the parentheses. We will use coding introduced above to take the mean of a specific rows or columns from a data frame.
...
```{r}
#install the matrixStats package
#install.packages("matrixStats")
#You will have to remove the comment from the above line to use.

#load the matrixStats package
library(matrixStats)
```
Install the matrixStats package and use the following functions to generate summary statistics for the lmb_byrow matrix created above.
